...
const TodaysMarket = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Attempt to fetch from AgMarkNet API
        const response = await fetch('https://agmarknet.gov.in/PriceAndArrivals/commoditywisedailyreport3.aspx', {
          method: 'GET',
          mode: 'cors'
        });
        ...
        // Use sample data if API fails (common with government APIs)
        setData(sampleData);
      } finally {
        setLoading(false);
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
            ...
            <div key={index} className="bg-white rounded-xl shadow-md p-4 hover:shadow-lg transition-all duration-300 border border-gray-100">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-medium ${
                  ...
                  commodity.change < 0 ? 'text-red-600' : 'text-gray-500'
                }`}>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={currentData.commodities} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip 
                ...
                formatter={(value, name) => [formatPrice(value), name === 'minPrice' ? 'Min Price' : name === 'maxPrice' ? 'Max Price' : 'Modal Price']}
              />
        ...
        <div className="text-center text-sm text-gray-500">
          ...
          <p className="mt-1">Last updated: {new Date().toLocaleDateString('en-IN', { 
            weekday: 'long', 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric' 
